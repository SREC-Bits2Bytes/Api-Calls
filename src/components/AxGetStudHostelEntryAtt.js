import React, { useEffect } from 'react';
import axios from 'axios';

<script src="https://js.stripe.com/v3/"></script>



const api = 'https://p2nzd3xj7i.execute-api.ap-south-1.amazonaws.com/dev/staff/fetch-student-hostel-entry-attendance?rollno=1901044&date=Aug 04 2022';
const token = JSON.parse(sessionStorage.getItem('data'));
//const tokens = token.data.subject;

export default function AxGetStudHostelEntryAtt() {
  const fetchData = async () => {
    try {
      const response = await axios.get(api, {
        headers: {
          "Access-Token": 'eyJraWQiOiJ0eU9LVHFIV1J2UDlNOGRVNDVQaHhRZkZDTndqRDQ4R3ltbVg4RDhmVmpFPSIsImFsZyI6IlJTMjU2In0..rTh_L1dr08xL3DqLMOXZumn4Nund2YSc0JizLZktuGlDaK9LtM42UAkd1iO9E270c8WG1W9wejM5Jq0kDu11HWCcZ78FB9Sd1CeQ5ruxsTbw6KdeSpBhwbJh4nhPG0PNq4zOglqkCiCDS-k1FjAY8Fd9Vce0RmLghnMjpj9Wlcu25R_wlDpG3lwLxpG1O_wsSLZDFDGExOwi4dQtlAdZM1T07KgCnJeSRaATc9HMDjGJWmqBSEr6jk2PYjonyMzpzE5h0GO3fQNCiqN-Nxst9o1kEnKBcZAe7yv9KTAjZwx9FSDRJZHpvrGTZP5AcjFiGAZWKneZsCKw0KwOnMm87g'
         //  ,
          //  'access-control-allow-origin': '*',
          // "Access-Control-Allow-Headers": "X-Requested-With"
        }
      });
      if (response.data) {
        console.log(response.data);
      } else {
        console.error("error");
      }
    } catch (error) {
      console.error(error);
    }
  }

  useEffect(() => {
    fetchData();
  }, []);
  return <div>Hi</div>
}
